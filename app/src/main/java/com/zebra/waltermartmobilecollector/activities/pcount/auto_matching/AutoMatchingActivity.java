...
public class AutoMatchingActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pcount_auto_matching);
    ...
    private void saveAutoMatchingReport() throws Exception {
        AMModel amModel = ReportService.getWithoutPas3(allData);
        isMatched = amModel.isMatched();

        reportFolder = (isMatched ? Folders.MATCHED_PCOUNT : Folders.UNMATCHED_PCOUNT) + Globals.selectedLocation + "/";

        FTP.getFtp().makeDirectory(reportFolder);
        moveFiles();
        if (isMatched) {
            runOnUiThread(()-> ((TextView) findViewById(R.id.txtTotalPcs)).setText(amModel.getTotalPcsExpected() + ""));
            FTP.copy(
            ...
            FTP.upload(reportFolder + Globals.selectedLocation + "_Report_Unmatched.csv", amModel.getReport());
        }
    ...
    private void moveFiles() throws Exception {
        ...
        FTP.move(
                Folders.SCANNED_PCOUNT + pas2Filename,
...
//                    Model model = Service.getPas3Model(rows.get(2), allData);
//                    if (model == null) {
//                        model = Service.getPas3Model(rows.get(2));
//                        allData.add(model);
//                    }
//                    if (model == null) return true;
//
...
//                    return true;
    ...
    private void saveReport() throws Exception {
        AMModel amModel = ReportService.get(allData);
