...
import com.zebra.waltermartmobilecollector.activities.pcount.auto_matching.single.SingleAutoMatchingActivity;
import com.zebra.waltermartmobilecollector.activities.purchase_order.auto_matching.POFilename;
...
public class ListActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.textView30)).setText("Location List");

        RecyclerView recyclerView = findViewById(R.id.recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        adaptor = new ListAdaptor(model -> {
            Intent intent = new Intent(
                    ListActivity.this,
                    !Globals.singleValidation
                            ? AutoMatchingActivity.class
                            : SingleAutoMatchingActivity.class
            );
            intent.putExtra("pas1_filename", model.getPas1());
            if (!Globals.singleValidation)
                intent.putExtra("pas2_filename", model.getPas2());
            Globals.selectedLocation = model.getPo();
...
//            folder = Folders.PER_CASE_PCOUNT;
        ...
        if (Globals.singleValidation) {
            runThread(() -> {
                ...
                for (FTPFile file : FTP.getFiles(Folders.SCANNED_PCOUNT)) {
                    if (!file.isFile()) continue;

                    String[] fn = file.getName().split("\\.");
                    if (fn.length < 2 || !fn[1].equals("txt")) continue;
                    String[] sp = fn[0].split("_");
                    if (sp.length != 3 || !sp[1].equals(identifier)) continue;
        ...
        runThread(() -> {
            ...
            for (FTPFile file : FTP.getFiles(Folders.SCANNED_PCOUNT)) {
                ...
                if (fn.length < 2 || !fn[1].equals("txt")) continue;
                String[] sp = fn[0].split("_");
                if (sp.length != 4 || !sp[1].equals(identifier)) continue;
